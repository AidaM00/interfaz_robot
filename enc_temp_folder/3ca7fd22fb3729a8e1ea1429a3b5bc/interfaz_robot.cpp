...
void interfaz_robot::CinematicaInversa(double cx, double cy, double cz, double angulo)
{
    ...
    // Dos soluciones
    double q3a = acos(arg_q3);
    double q3b = -acos(arg_q3);

    // --- Q2 para la solución A ---
    double A = a2 + a3 * cos(q3a);
    double B = a3 * sin(q3a);
    double arg_q2a = (X * A + B * r) / (A * A + B * B);
    arg_q2a = std::clamp(arg_q2a, -1.0, 1.0);
    double q2a = acos(arg_q2a);
    double q4a = M_PI - (q2a + q3a);

    // --- Q2 para la solución B ---
    A = a2 + a3 * cos(q3b);
    B = a3 * sin(q3b);
    double arg_q2b = (X * A + B * r) / (A * A + B * B);
    arg_q2b = std::clamp(arg_q2b, -1.0, 1.0);
    double q2b = acos(arg_q2b);
    double q4b = M_PI - (q2b + q3b);

    // Aquí decides qué solución usar:
    bool usarSolA = false;  // o elige según límites de tu robot

    double q1 = atan2(cy, cx);
