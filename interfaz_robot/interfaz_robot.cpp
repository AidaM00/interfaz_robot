...
void interfaz_robot::getNewFrame()
{
    ...
    if (m_comenzarProcesado)
    {
        ...
        if (m_cogerPieza)
        {
            ...
            float angulo = atan2(
                centro_baseRobot.x - lejano_baseRobot.x,
                centro_baseRobot.y - lejano_baseRobot.y
            ) * 180.0 / M_PI;
...
PuntosProcesados interfaz_robot::ComenzarProcesado(Mat img)
{
    ...
    std::vector<cv::Point2f> srcPoints = {
        cv::Point2f(470, 254),    // Superior izquierda
        cv::Point2f(1316, 269),   // Superior derecha
        cv::Point2f(273, 1050),    // Inferior izquierda
        cv::Point2f(1488, 1046)   // Inferior derecha
    };
